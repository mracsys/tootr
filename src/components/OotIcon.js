...
const OotIcon = (props) => {
    ...
    return (
        <div onClick={props.onClick ? props.onClick : null} data-source={props.dataSource} className="iconDiv">
            <div className={props.className}>
            <img
                src={iconMap[props.itemName].src}
                alt={props.itemName}
                className="iconContainer"
            />
            { 
                props.itemName.includes('Small Key') ?
                    <span className="iconKeyText">{keyMap[props.itemName.substring(11, props.itemName.length - 1)]}</span>
                    : null
            }
            { 
                props.itemName.includes('Boss Key') ?
                    <span className="iconKeyText">{keyMap[props.itemName.substring(10, props.itemName.length - 1)]}</span>
                    : null
            }
            { 
                props.itemName.includes('Silver Rupee') ?
                    <span className="iconKeyText">{rupeeMap[props.itemName.substring(14, props.itemName.length - 1)]}</span>
                    : null
            }
            { 
                (props.itemName in songMap) ?
                    <span className="iconKeyText">{songMap[props.itemName]}</span>
                    : null
            }
            { 
                props.itemName === 'Chicken' ?
                    <span className="iconTradeText">C</span>
                    : null
            }
            { 
                props.itemName === 'Pocket Cucco' ?
